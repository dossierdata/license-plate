...
class GermanLicensePlate extends AbstractLicensePlate implements LicensePlateInterface
{
    ...
    public function format($sidecode = 0)
    {
        ...
        switch ($sidecode) {
            ...
            case self::SIDE_CODE_ELECTRICAL:
                $formatted_license_plate = $this->formatDefaultLicensePlate(substr($original_transformed_licenseplate,
                        0, -1), substr($licenseplate, 0, -1)) . 'E';
                return $formatted_license_plate;

            case self::SIDE_CODE_HISTORICAL:
                $formatted_license_plate = $this->formatDefaultLicensePlate(substr($original_transformed_licenseplate,
                        0, -1), substr($licenseplate, 0, -1)) . 'H';
                return $formatted_license_plate;
            ...
            case self::SIDE_CODE_AMERICAN_FORCES:
                ...
                return substr($licenseplate, 0, 2) . ' ' . substr($licenseplate, 2,
                        $first_number_index - 2) . ' ' . substr($licenseplate, $first_number_index);
    ...
    private function formatDefaultLicensePlate($original_transformed_licenseplate, $licenseplate)
    {
        ...
        if ($delimiter_position !== false) {
            $formatted_license_plate = $this->getFormattedLicensePlateForLicensePlateWithFormat($original_transformed_licenseplate,
                $licenseplate, $delimiter_position);
    ...
    private function getFormattedLicensePlateForLicensePlateWithFormat(
